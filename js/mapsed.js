...
(function () {
	...
	$.fn.mapsed = function (options) {
		...
		function buildMsg(title, msg, doConfirm, prompt, callback) {
			...
			if (doConfirm) {
				buttons += "<button class='ok'>Cerrar</button>";
				buttons += "<button class='cancel'>Cancel</button>";
			} else {
				buttons += "<button class='close'>Cerrar</button>";
			}
			buttons += "</div>";
			buttons += "</div>";
		...
		function applyTemplate(tmpl, model, $ctx) {
		    tmpl = replaceAll("{Finca}", model.Finca, tmpl);
		    tmpl = replaceAll("{SHORT_NAME}", shorten(model.Finca, 25), tmpl);
		    tmpl = replaceAll("{lat}", model.lat, tmpl);
		    tmpl = replaceAll("{lng}", model.lng, tmpl);
		    tmpl = replaceAll("{TipoApoyo}", model.TipoApoyo, tmpl);
		    tmpl = replaceAll("{Cultivo}", model.Cultivo, tmpl);
		    tmpl = replaceAll("{Beneficiario}", model.Beneficiario, tmpl);
		    tmpl = replaceAll("{Tecnico}", model.Tecnico, tmpl);
			...
			tmpl = replaceAll("{NumeroLote}", model.NumeroLote, tmpl);
			tmpl = replaceAll("{Hectareas}", model.Hectareas, tmpl);
			if (model.photo) {
		...
		function hideEmpty(model, $vw) {
			...
			$vw.find(".mapsed-lat").toggle(model.street && model.street.length > 0);
			...
			var $telNo = $vw.find(".mapsed-telNo"),
			        ...
			        $Beneficiario = $vw.find(".mapsed-Beneficiario"),
                    $Tecnico = $vw.find(".mapsed-Tecnico")
			;
		...
		function showTooltip(inRwMode) {
			...
			var msg = getPlaceHtml(marker.details);
			_plugIn.showMsg("Información del lote", msg);
		...
		function getViewTemplate() {
			...
			var html =
			  ...
			  + "<tr>"
			  + "<td colspan='3'>"
			  + "<h1 class='mapsed-name' title='{Finca}'>{Finca}</h1>"
			  + "</td>"
			  + "</tr>"
			  + "<tr>"
			  + "<td class='mapsed-left'>"
			  + "<address>"
			  + "<div class='mapsed-lat'>Latitud: {lat}</div>"
              + "<div class='mapsed-lng'>Longitud: {lng}</div>"
              + "<div class='mapsed-TipoApoyo'>{TipoApoyo}</div>"
              + "<div class='mapsed-Cultivo'>{Cultivo}</div>"
              + "<div class='mapsed-NumeroLote'>{NumeroLote}</div>"
              + "<div class='mapsed-Hectareas'>{Hectareas}</div>"
			  + "<div class='mapsed-town'>{TOWN}</div>"
			  + "<div class='mapsed-area'>{AREA}</div>"
			  + "<div class='mapsed-postCode'>{POSTCODE}</div>"
			  + "</address>"
              + "<div class='mapsed-Beneficiario' style='display: none;'>{Beneficiario}</div>"
              + "<div class='mapsed-Tecnico' style='display: none;'>{Tecnico}</div>"
		...
		function getEditTemplate() {
			var html =
				...
				+ "<li>"
				+ "<label>Name"
				+ "<input class='mapsed-name' type='text' placeholder='e.g. Bob sandwich shop' value='{Finca}' />"
				+ "</label>"
				+ "</li>"
				+ "<li>"
				+ "<label>lat"
				+ "<input class='mapsed-lat' type='text' placeholder='e.g. 3 Hemington place' value='{lat}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>lng"
				+ "<input class='mapsed-lng' type='text' placeholder='e.g. 3 Hemington place' value='{lng}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>TipoApoyo"
				+ "<input class='mapsed-TipoApoyo' type='text' placeholder='e.g. 3 Hemington place' value='{TipoApoyo}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>Cultivo"
				+ "<input class='mapsed-Cultivo' type='text' placeholder='e.g. 3 Hemington place' value='{Cultivo}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>Cultivo"
				+ "<input class='mapsed-Hectareas' type='text' placeholder='e.g. 3 Hemington place' value='{Hectareas}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>Beneficiario"
				+ "<input class='mapsed-Beneficiario' type='text' placeholder='e.g. 3 Hemington place' value='{Beneficiario}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>Tecnico"
				+ "<input class='mapsed-Tecnico' type='text' placeholder='e.g. 3 Hemington place' value='{Tecnico}' />"
				+ "</label>"
				+ "</li>"
                + "<li>"
				+ "<label>Town"
		...
		function getViewModel($vw) {
			...
			if ($vw.hasClass("mapsed-view")) {
				...
				model.Beneficiario = $vw.find(".mapsed-Beneficiario").html();
				model.Tecnico = $vw.find(".mapsed-Tecnico").html();
				model.website = $vw.find(".mapsed-website").attr("href");
				model.url = $vw.find(".mapsed-url").attr("href");
				model.NumeroLote = $vw.find(".mapsed-NumeroLote").html();
				model.TipoApoyo = $vw.find(".mapsed-TipoApoyo").html();
				model.Cultivo = $vw.find(".mapsed-Cultivo").html();
				model.Hectareas = $vw.find(".mapsed-Hectareas").html();
			} else {
				// editor view
				model.name = $vw.find(".mapsed-name").val();
				model.lat = $vw.find(".mapsed-lat").val();
				...
				model.Beneficiario = $vw.find(".mapsed-Beneficiario").val();
				model.Tecnico = $vw.find(".mapsed-Tecnico").val();
		...
		function sanitise(place) {
			...
			if (!place.Beneficiario) place.Beneficiario = "";
			if (!place.Tecnico) place.Tecnico = "";
		...
		function normalisePlacesApiAddress(details, fromGP) {
			...
			details.Beneficiario = fromGP.Beneficiario || "";
			details.Tecnico = fromGP.Tecnico || "";
